...
void pipeline::run() { this->command_child->run(); }

void conditional::run() {
  pipeline *pipeline = this->pipeline_child;
...
command *parse_line(const char *s) {
  ...
  command *ccur = nullptr, *next_command = nullptr, *chead = nullptr;
  pipeline *p = nullptr;
  conditional *cond = nullptr;
  (void)ccur, (void)p, (void)cond;
  ...
  for (shell_token_iterator it = parser.begin(); it != parser.end(); ++it) {
    ...
    case TYPE_SEQUENCE:
      ...
      if (tempIt != parser.end()) {
        ...
        ccur->next_in_pipeline = next_command;
        ccur = next_command;
      }
      break;
    case TYPE_OR:
  ...
  while (!feof(command_file)) {
    ...
    if (bufpos == BUFSIZ - 1 || (bufpos > 0 && buf[bufpos - 1] == '\n')) {
      if (command *c = parse_line(buf)) {
        /* i made for test : for (auto elt : c->args)
          std::cout << elt << "\n"; */
        run_list(c);
        delete c;
      }
